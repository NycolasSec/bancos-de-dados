-- Active: 1706635174833@@127.0.0.1@3306@db_biblioteca_2 USE db_biblioteca_2;
DROP TABLE livro;

CREATE TABLE livro (
    id_livro INT PRIMARY KEY NOT NULL AUTO_INCREMENT, nome_livro VARCHAR(120), uf_livro VARCHAR(2), valor_livro DECIMAL(10, 2), num_pagina_livro INT, ano_publicacao_livro DATE, nome_editora VARCHAR(120), nome_autor VARCHAR(120), sexo_autor VARCHAR(1)
);
...
SELECT nome_livro, uf_livro
FROM livro
WHERE
    sexo = 'm';

SELECT nome_livro, num_pagina_livro
FROM livro
WHERE
    sexo = 'm';
...
SELECT nome_autor, sexo, uf_livro
FROM livro
WHERE (
        uf_livro = 'SP'
        OR uf_livro = 'RJ'
...
SELECT uf_livro, COUNT(*) AS quantidade
FROM livro
GROUP BY
    uf_livro;
...
SELECT AVG(valor_livro) FROM livro;

SELECT SUM(valor_livro) / COUNT(*) FROM livro;

SELECT MAX(valor_livro) FROM livro;

SELECT MIN(valor_livro) FROM livro;

SELECT MAX(valor_livro), MIN(valor_livro) FROM livro;

SELECT
    SUM(valor_livro) AS ValorTotal,
    (SUM(valor_livro) *.1) AS Desconto,
    SUM(valor_livro) - (SUM(valor_livro) *.1) AS ValorComDesconto
FROM
    livro;